...
def graafik():
    ...
    if graf==True:
        ...
        #lbl4.configure(text=text2)
    else:
        text=f"Нет возможности построить график"
def veel():
    global t
    if t==0:
        aken.geometry(str(aken.winfo_width())+"x"+str(aken.winfo_height()-170))
...
aken.geometry("500x225")
f1=Frame(aken,width=500,heigh=170)
f1.pack(side=TOP)
f2=Frame(aken,width=500,heigh=170)
f2.pack(side=BOTTOM)
...
nupp2=Button(f1,text="График",font="Arial 19",height=1,width=6,bg="green",fg="black",relief=RAISED,command=graafik)
...
var=IntVar()
r1=Radiobutton(f2,text="Кит",variable=var,value=1, font="Arial 20",command=figura)
r2=Radiobutton(f2,text="Очки",variable=var,value=2, font="Arial 20",command=figura)
r3=Radiobutton(f2,text="Бабочка",variable=var,value=3, font="Arial 20",command=figura)
r4=Radiobutton(f2,text="Зонтик",variable=var,value=4, font="Arial 20",command=figura)
r1.pack()
r2.pack()
r3.pack()
r4.pack()
